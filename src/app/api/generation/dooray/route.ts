...
import { generateDoorayReport } from '@/lib/ai-generation';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rangeDays = typeof body?.rangeDays === 'number' && body.rangeDays > 0 ? body.rangeDays : 14;
    const db = getDb();
    ...
    const template = templateId
      ? db.prepare('SELECT content FROM generation_templates WHERE id = ? AND type = ?').get(templateId, 'dooray') as { content: string } | undefined
      : db.prepare('SELECT content FROM generation_templates WHERE type = ? ORDER BY is_default DESC, updated_at DESC LIMIT 1').get('dooray') as { content: string } | undefined;
    ...
    const bestPractices = db.prepare(`
      ...
      LIMIT 50
    `).all('dooray') as Array<{ title: string; content: string }>;

    const markdown = await generateDoorayReport({
